...
{
	return abs(pt(A, B).cp(pt(b.A, b.B))) > eps;
}

bool line::operator==(const line &b) const
{
	return abs(A * b.B - b.A * B) < eps
		&& abs(A * b.C - b.A * C) < eps
		&& abs(B * b.C - b.B * C) < eps;
}
...
bool poly::cont(const pt &a) const
{
	...
	if (!ok)
	{
		...
		if ((int)round(abs(an / (2 * the_PI))) == 1)
		{
			ok = true;
		}
		else
...
bool poly::cont_log(const pt & a) const
{
	...
	if (it == polar.end() && abs(an - polar.back()) < eps)
	{
		it = polar.end() - 1;
	}
...
void poly::cont_log_init()
{
	...
	{
		if (pts[i].x < zero.x || (abs(pts[i].x - zero.x) < eps && pts[i].y < zero.y))
		{
			zero = pts[i];
			i_zero = i;
		}
...
bool poly::isConvex()
{
	...
	{
		if (pt(p[i], p[i - 1]).angle2(pt(p[i], p[i + 1])) > the_PI + eps)
		{
			ok = false;
		}
	}
...
void poly::reverseCW()
{
	...
	bool rev = abs(an - the_PI * (n - 2)) < eps;
	if (!rev)
	{
...
vector<pt> krug::operator*(const krug &b) const
{
	...
	if (abs(d) < eps && abs(rad - b.rad) < eps)
	{
		// INF int
		return vector<pt>(3);
	}
	else if (abs(d - (rad - b.rad)*(rad - b.rad)) < eps || abs(d - (rad + b.rad)*(rad + b.rad)) < eps)
	{
		// 1 kas
		pt O1O2(center, b.center);
		if (abs(d - (rad - b.rad)*(rad - b.rad)) < eps && rad < b.rad)
		{
			// INNER case
			O1O2 = -O1O2;
		}
